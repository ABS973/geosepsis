...
class SimulationDataSource extends DataSource{
 constructor(random=Math.random){super();this.random=random;this.pace=.9+random()*.2;}
 normal(){return Math.sqrt(-2*Math.log(Math.max(1e-12,this.random())))*Math.cos(2*Math.PI*this.random());}
 next(t=0){const d=clamp((t-15)/(75*this.pace),0,1),z=Array.from({length:9},()=>this.normal()),f=z[0],b=z[1],amp=1+1.8*d;
 const hr=clamp(76+10*d+amp*(1.8*z[2]+(.35+2.8*d)*f),52,135),rr=clamp(15+2.7*d+amp*(.65*z[3]+(.12+1.0*d)*f),9,30),o=clamp(98-.9*d+.18*z[4]-(.03+.28*d)*f,91,100),temp=clamp(36.8+.65*d+.065*z[5]+(.015+.16*d)*f,35.5,39.5),sbp=clamp(117-9*d+2.1*z[6]+1.4*b-3.8*d*f,88,145),dbp=clamp(74-4*d+1.35*z[7]+1.1*b-2*d*f,52,95);
 return [hr,rr,o,temp,sbp,dbp,(sbp+2*dbp)/3,sbp-dbp];}}
class Engine{
 constructor(random=Math.random){this.source=new SimulationDataSource(random);this.t=0;this.id=null;this.events=[];this.warning=false;this.history=[];this.states=[];this.peak=0;
